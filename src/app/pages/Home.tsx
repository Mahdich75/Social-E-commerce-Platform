...
export default function Home() {
  ...
  const dynamicComments = useMemo(() => {
    ...
    const productComments = [
      `${product.name} خیلی خوبه، کسی تجربه استفاده داره؟`,
      `از @${product.creatorUsername} کسی این مدل ${product.category} رو خریده؟`,
      `کیفیت ${product.name} نسبت به قیمتش چطوره؟`,
      `ارسال @${product.creatorUsername} به شهرستان هم انجام میشه؟`,
      `برای استفاده روزمره ${product.name} انتخاب خوبیه؟`,
    ];
